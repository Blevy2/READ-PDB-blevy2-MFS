...
# #write csvs
write.csv(sum_survey_iter_final[[1]], file="YT_SRS_GB_excludestrata_DecPop_ConTemp.csv", row.names=F)
write.csv(sum_survey_iter_final[[2]], file="Cod_SRS_GB_excludestrata_DecPop_ConTemp.csv", row.names=F)
write.csv(sum_survey_iter_final[[3]], file="Had_SRS_GB_excludestrata_DecPop_ConTemp.csv", row.names=F)
...
res[["survey"]][["log.mat"]] <- surv_random$log.mat
...
for(s in short_names){ 
  path <- "Results/DecPop_ConTemp/"
  SRS_data[[s]] <- read_csv(file = paste(path,s,"_SRS_GB_allstrata_DecPop_ConTemp.csv",sep=""))
