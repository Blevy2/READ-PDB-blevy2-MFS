...
gis.name <- "C:\\Users\\benjamin.levy\\Desktop\\NOAA\\GIS_Stuff\\Plot_survey\\ADIOS_SV_164712_GBK_NONE_survey_dist_map_fixed.csv"
...
#save
saveRDS(final_matrix,file="TestScripts/Habitat_plots/Cod_Weighted_AdaptFalse_MATRIX.RDS")

saveRDS(final_ras,file="TestScripts/Habitat_plots/Cod_Weighted_AdaptFalse_RASTER.RDS")


# #rescale raster
# test<-raster::aggregate(final_ras,fact=1.5,na.rm=T)
# cod_mtrx<-as.matrix(test)
# #count zeros
# 
# sum(colSums(cod_mtrx>=0,na.rm=TRUE),na.rm=TRUE) #total non-NA
# sum(colSums(cod_mtrx>0,na.rm=TRUE),na.rm=TRUE) #total nonzero
# sum(colSums(cod_mtrx==0,na.rm=TRUE),na.rm=TRUE)#total zero
# 
# sum(colSums(had_mtrx>=0,na.rm=TRUE),na.rm=TRUE) #total non-NA
# sum(colSums(had_mtrx>0,na.rm=TRUE),na.rm=TRUE) #total nonzero
# sum(colSums(had_mtrx==0,na.rm=TRUE),na.rm=TRUE)#total zero
