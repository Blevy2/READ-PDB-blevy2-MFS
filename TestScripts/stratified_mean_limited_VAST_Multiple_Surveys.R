...
strata_surv <- vector("list",length=length(surv_random_list))
#pull out each strata survey info and store separately  


for(iter in seq(length(surv_random_list))){

temp <- surv_random_list[[iter]]$log.mat
idx <- 1
for(i in seq(nstrata)){
  
  strata_surv[[iter]][[i]] <- temp[idx:(idx+strat_samp_tot[i]-1),1:n_cols]
...
for(i in seq(nstrata)){
  ...
  # print(swks)
  for(iter in seq(length(surv_random_list))){
  strata_surv[[iter]][[i]] <- cbind(strata_surv[[iter]][[i]],swks,Season)
  colnames(strata_surv[[iter]][[i]]) <- c("station_no","x","y","stratum","day","tow","year",paste0("spp", seq_len(n_spp)),"week","Season")
  }
...
list_all <- vector("list",length(surv_random_list))


for(iter in seq(length(surv_random_list))){#for each simulation result

  
  for(s in seq(nstrata)){  #pull out strata
    
    for(i in seq(length(strata_surv[[iter]][[s]][,1]))){ #go down list
      
      x <- as.numeric(strata_surv[[iter]][[s]][i,"x"])   #x coord in second column
      y <- as.numeric(strata_surv[[iter]][[s]][i,"y"])   #y coord in third column  
      year <- as.numeric(strata_surv[[iter]][[s]][i,"year"]) #year in 7th column
      week <- as.numeric(strata_surv[[iter]][[s]][i,"week"])  #appended sample week into 10th column above
  ...
  for(i in seq(nstrata)){
    assign(paste("all_",i,"_",iter,sep=""),strata_surv[[iter]][[i]])
    
    list_all[[iter]] <-rbind(list_all[[iter]],strata_surv[[iter]][[i]])
  }
