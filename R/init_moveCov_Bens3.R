...
init_moveCov_Bens3 <- function (sim_init = NULL, steps = NULL, spp_tol = NULL) {	
	...
	val <- 1
      ...
      if(i %% 52 != 0){
        ...
        #half the year increase and half the year decrease
        ...
        if(i %% 52 < 27){  
			covariate.trend <- ifelse(row(temp_original) + col(temp_original) < ((nx + ny) / 2), #this creates a matrix where the upper half is true and the lower half is false going along the antidiagonal
						  covariate.trend_base + (3 * ((i%%52)/27)),  #52 used to be steps
						  covariate.trend_base - (6 * ((i%%52)/27)))
			cov.matrix[[i]] <- covariate.trend
        }
        
        
        #increase
        #first week reset covariate base matrix
        if(i %% 52 == 27){covariate.trend_base <- covariate.trend}
        
        
        if(i %% 52 >= 27){  
          covariate.trend <- ifelse(row(temp_original) + col(temp_original) < ((nx + ny) / 2), #this creates a matrix where the upper half is true and the lower half is false going along the antidiagonal
                                    covariate.trend_base - (3 * (((i%%52)-26)/26)),  #52 used to be steps
                                    covariate.trend_base + (6 * (((i%%52)-26)/26)))
          cov.matrix[[i]] <- covariate.trend
        }
