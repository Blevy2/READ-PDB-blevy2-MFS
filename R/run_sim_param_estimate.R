...
run_sim_param_estimate <- function(x) {
...
print("New Iteration")
        ...
        #  print(s)
          ...
          if(year.breaks[t]==1){  #NEED TO ACCOUNT FOR FIRST YEAR
            ...
            ifelse( t == tail(pop_init[["dem_params"]][[s]][["rec_wk"]],n=1),
                    ...
                    {SB <- sum(B[[s]],na.rm=T)
                    rec1 <- length(pop_init[["dem_params"]][[s]][["rec_wk"]])*Recr(model = "BH",
                                                                                      params = c("a" = a,
                                                                                                 "b" = b),
 ...
 rec
    ...
    if(t%%7 == 1){#fish on first day of week
      ...
      B_new <- list(matrix(0, ncol = ncols, nrow = nrows))
      names(B_new) <- paste0("spp", seq_len(n_spp))
      ...
      if(t==1) {
        ...
        catches <- lapply(seq_len(n_fleets), function(fl) { 
          ...
          go_fish_fleet(FUN = go_fish, 
                        ...
                        closed_areas = AreaClosures, pops = B_new, t = t)
        })
        #print("catches")
        #print(catches)
      } # end t==1 run
        ...
        if(day.breaks[t] == day.breaks[t-1]) {
          catches <- lapply(seq_len(n_fleets), function(fl) {  
            ...
            go_fish_fleet(FUN = go_fish, 	
                          ...
                          pops = B_new, t = t, closed_areas = AreaClosures)
          })
